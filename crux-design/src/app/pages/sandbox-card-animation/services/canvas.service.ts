...
export interface ShapeData {
  ...
  radius?: number;
  animation?: ShapeAnimation; // Add animation property
}
...
export class CanvasController {
  ...
  private drawStoredShapes(): void {
    ...
    for (const shape of this.shapes) {
      ...
      // Apply animation transforms if present
      if (shape.animation) {
        const animTransform = this.getAnimationTransform(shape.animation);
        
        // Calculate shape center for transformations
        const shapeCenter = this.getShapeCenter(shape, transformedPosition);
        
        // Apply transformations in order: translate to center, transform, translate back
        this.ctx.translate(shapeCenter.x, shapeCenter.y);
        
        // Apply shear
        this.ctx.transform(1, animTransform.shear.y, animTransform.shear.x, 1, 0, 0);
        
        // Apply scale
        this.ctx.scale(animTransform.scale.x, animTransform.scale.y);
        
        // Apply rotation
        this.ctx.rotate(animTransform.rotation);
        
        // Apply translation
        this.ctx.translate(animTransform.translation.x * this.scale, -animTransform.translation.y * this.scale);
        
        // Translate back to shape position
        this.ctx.translate(-shapeCenter.x, -shapeCenter.y);
      }
  ...
  private getShapeCenter(shape: ShapeData, transformedPosition: Point): Point {
    if (shape.type === 'circle' && shape.points.length === 1) {
      ...
      for (const point of shape.points) {
        sumX += transformedPosition.x + point.x * this.scale;
        sumY += transformedPosition.y - point.y * this.scale;
      }
      return new Point(sumX / shape.points.length, sumY / shape.points.length);
    }
  ...
  private updateAnimations(deltaTime: number): void {
    for (const shape of this.shapes) {
      if (shape.animation && shape.animation.isPlaying) {
        shape.animation.currentTime += deltaTime;
        
        if (shape.animation.currentTime >= shape.animation.duration) {
          if (shape.animation.loop) {
            shape.animation.currentTime = shape.animation.currentTime % shape.animation.duration;
          } else {
            shape.animation.currentTime = shape.animation.duration;
            shape.animation.isPlaying = false;
          }
        }
      }
    }
  ...
  private getAnimationTransform(animation: ShapeAnimation): {
    translation: { x: number; y: number };
    rotation: number;
    scale: { x: number; y: number };
    shear: { x: number; y: number };
  } {
    if (!animation.isPlaying || animation.keyframes.length === 0) {
      return {
        translation: { x: 0, y: 0 },
        rotation: 0,
        scale: { x: 1, y: 1 },
    ...
    for (let i = 0; i < animation.keyframes.length - 1; i++) {
      if (currentTime >= animation.keyframes[i].time && currentTime <= animation.keyframes[i + 1].time) {
    ...
    return {
      translation: {
        x: this.lerp(prevKeyframe.translation?.x || 0, nextKeyframe.translation?.x || 0, progress),
        y: this.lerp(prevKeyframe.translation?.y || 0, nextKeyframe.translation?.y || 0, progress)
      },
      rotation: this.lerp(prevKeyframe.rotation || 0, nextKeyframe.rotation || 0, progress),
      scale: {
        x: this.lerp(prevKeyframe.scale?.x || 1, nextKeyframe.scale?.x || 1, progress),
        y: this.lerp(prevKeyframe.scale?.y || 1, nextKeyframe.scale?.y || 1, progress)
      },
      shear: {
        x: this.lerp(prevKeyframe.shear?.x || 0, nextKeyframe.shear?.x || 0, progress),
        y: this.lerp(prevKeyframe.shear?.y || 0, nextKeyframe.shear?.y || 0, progress)
      }
    };
  }
  ...
  public addAnimationToShape(shapeId: string, animation: ShapeAnimation): boolean {
  ...
  public removeAnimationFromShape(shapeId: string): boolean {
    const shape = this.shapes.find(s => s.id === shapeId);
    if (!shape) return false;
  ...
  public playShapeAnimation(shapeId: string): boolean {
    const shape = this.shapes.find(s => s.id === shapeId);
    if (!shape || !shape.animation) return false;
    
    shape.animation.isPlaying = true;
    shape.animation.currentTime = 0;
  ...
  public pauseShapeAnimation(shapeId: string): boolean {
    const shape = this.shapes.find(s => s.id === shapeId);
    if (!shape || !shape.animation) return false;
  ...
  public stopShapeAnimation(shapeId: string): boolean {
    const shape = this.shapes.find(s => s.id === shapeId);
    if (!shape || !shape.animation) return false;
...
export class CanvasService {
  ...
  public addAnimationToShape(shapeId: string, animation: ShapeAnimation): boolean {
    if (this.controllerInstance) {
      return this.controllerInstance.addAnimationToShape(shapeId, animation);
    }
    return false;
  }
  ...
  public removeAnimationFromShape(shapeId: string): boolean {
    if (this.controllerInstance) {
      return this.controllerInstance.removeAnimationFromShape(shapeId);
    }
    return false;
  }
  ...
  public playShapeAnimation(shapeId: string): boolean {
    if (this.controllerInstance) {
      return this.controllerInstance.playShapeAnimation(shapeId);
    }
    return false;
  }
  ...
  public pauseShapeAnimation(shapeId: string): boolean {
    if (this.controllerInstance) {
      return this.controllerInstance.pauseShapeAnimation(shapeId);
    }
    return false;
  }
  ...
  public stopShapeAnimation(shapeId: string): boolean {
    if (this.controllerInstance) {
      return this.controllerInstance.stopShapeAnimation(shapeId);
    }
    return false;
  }
  ...
  public startAnimations(): void {
    if (this.controllerInstance) {
      this.controllerInstance.startAnimation();
    }
  }

  public stopAnimations(): void {
    if (this.controllerInstance) {
      this.controllerInstance.stopAnimation();
    }
  }
}
