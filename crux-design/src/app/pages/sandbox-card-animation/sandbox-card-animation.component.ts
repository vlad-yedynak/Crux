...
import { CanvasService, Point, ShapeData, AnimationKeyframe, ShapeAnimation, validateSquarePoints, validateTrianglePoints, validateRectanglePoints, validateCircleData, validatePolygonPoints } from './services/canvas.service'; 
...
@Component({
  ...
  templateUrl: './sandbox-card-animation.component.html',
  styleUrl: './sandbox-card-animation.component.css',
...
export class SandboxCardAnimationComponent implements OnInit, AfterViewInit, OnDestroy {
  card: Card | null = null; // Uses Card from LessonsService
  private canvasInitialized = false;
  activePanel: 'shapes' | 'tasks' | 'animations' = 'shapes';  // Removed 'curves'
  ...
  selectedAnimationShapeId: string | null = null;
  animationDuration: number = 5;
  animationLoop: boolean = true;
  keyframes: AnimationKeyframe[] = [];
  editingKeyframeIndex: number | null = null;
  ...
  ngOnDestroy(): void {
    // Stop animations when component is destroyed
    this.canvasService.stopAnimations();
  ...
  private setupCanvasIfReady(): void {
    ...
    if (
      this.card?.sandboxType === 'Animation' &&
  ...
  getNonPlaceholderShapes(): ShapeData[] {
    return this.drawnShapes.filter(shape => !shape.isPlaceholder);
  }

  getSelectedShapeName(): string {
    if (!this.selectedAnimationShapeId) return '';
    const shape = this.drawnShapes.find(s => s.id === this.selectedAnimationShapeId);
    return shape?.name || shape?.type || '';
  }

  getAnimatedShapes(): ShapeData[] {
    return this.drawnShapes.filter(shape => shape.animation && !shape.isPlaceholder);
  }

  // Animation Methods
  switchPanel(panel: 'shapes' | 'tasks' | 'animations'): void {
    this.activePanel = panel;
  }

  selectShapeForAnimation(shapeId: string): void {
    this.selectedAnimationShapeId = shapeId;
    const shape = this.drawnShapes.find(s => s.id === shapeId);
    if (shape && shape.animation) {
      this.animationDuration = shape.animation.duration;
      this.animationLoop = shape.animation.loop;
      this.keyframes = [...shape.animation.keyframes];
    } else {
      this.animationDuration = 5;
      this.animationLoop = true;
      this.keyframes = [
        { time: 0, translation: { x: 0, y: 0 }, rotation: 0, scale: { x: 1, y: 1 }, shear: { x: 0, y: 0 } },
        { time: this.animationDuration, translation: { x: 0, y: 0 }, rotation: 0, scale: { x: 1, y: 1 }, shear: { x: 0, y: 0 } }
      ];
    }
  }

  addKeyframe(): void {
    const newTime = this.keyframes.length > 0 ? 
      Math.min(this.animationDuration, Math.max(...this.keyframes.map(kf => kf.time)) + 1) : 
      0;
  ...
  editKeyframe(index: number): void {
    this.editingKeyframeIndex = this.editingKeyframeIndex === index ? null : index;
  }

  updateKeyframe(index: number, property: string, value: string): void {
    if (index < 0 || index >= this.keyframes.length) return;
    
    const keyframe = this.keyframes[index];
    
    if (property === 'time') {
      keyframe.time = Math.max(0, Math.min(this.animationDuration, parseFloat(value)));
    } else if (property === 'translation.x') {
      if (!keyframe.translation) keyframe.translation = { x: 0, y: 0 };
      keyframe.translation.x = parseFloat(value);
    } else if (property === 'translation.y') {
      if (!keyframe.translation) keyframe.translation = { x: 0, y: 0 };
      keyframe.translation.y = parseFloat(value);
    } else if (property === 'rotation') {
      keyframe.rotation = parseFloat(value) * (Math.PI / 180); // Convert degrees to radians
    } else if (property === 'scale.x') {
      if (!keyframe.scale) keyframe.scale = { x: 1, y: 1 };
      keyframe.scale.x = Math.max(0.1, parseFloat(value));
    } else if (property === 'scale.y') {
      if (!keyframe.scale) keyframe.scale = { x: 1, y: 1 };
      keyframe.scale.y = Math.max(0.1, parseFloat(value));
    } else if (property === 'shear.x') {
      if (!keyframe.shear) keyframe.shear = { x: 0, y: 0 };
      keyframe.shear.x = parseFloat(value);
    } else if (property === 'shear.y') {
      if (!keyframe.shear) keyframe.shear = { x: 0, y: 0 };
      keyframe.shear.y = parseFloat(value);
    }
  ...
  // Helper methods for handling input events with proper typing
  onKeyframeTimeChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'time', target.value);
  }

  onKeyframeTranslationXChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'translation.x', target.value);
  }

  onKeyframeTranslationYChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'translation.y', target.value);
  }

  onKeyframeRotationChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'rotation', target.value);
  }

  onKeyframeScaleXChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'scale.x', target.value);
  }

  onKeyframeScaleYChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'scale.y', target.value);
  }

  onKeyframeShearXChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'shear.x', target.value);
  }

  onKeyframeShearYChange(index: number, event: Event): void {
    const target = event.target as HTMLInputElement;
    this.updateKeyframe(index, 'shear.y', target.value);
  }
  ...
  saveAnimation(): void {
    ...
    this.canvasService.addAnimationToShape(this.selectedAnimationShapeId, animation);
  ...
  playAnimation(shapeId: string): void {
    this.canvasService.playShapeAnimation(shapeId);
    this.canvasService.startAnimations();
  }

  pauseAnimation(shapeId: string): void {
    this.canvasService.pauseShapeAnimation(shapeId);
  }

  stopAnimation(shapeId: string): void {
    this.canvasService.stopShapeAnimation(shapeId);
  }

  removeAnimation(shapeId: string): void {
    this.canvasService.removeAnimationFromShape(shapeId);
    
    // Update local shape data
    const shape = this.drawnShapes.find(s => s.id === shapeId);
    if (shape) {
      delete shape.animation;
    }
